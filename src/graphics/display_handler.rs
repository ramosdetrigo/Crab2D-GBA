...
impl DisplayHandler {
    pub fn new() -> Self {
        DisplayHandler {
            vid_page: MEM_VRAM_ADDR as *mut u16,
        }
    }
    ...
    pub fn flip_page(&mut self) {
        self.vid_page = (self.vid_page.addr() ^ VRAM_PAGE_SIZE) as *mut u16;
    ...
    pub fn video_mode(&self) -> u32 {
        DISPCNT.read().video_mode() as u32
    }

    #[inline]
    /// Sets full display mode mask
    pub fn set_display_mode(&self, mode: DisplayControl) {
        DISPCNT.write(mode)
    }
    ...
    pub fn point(&self, p: Vec2<u32>, color: Color) {
        unsafe {
            let screen_width = match self.video_mode() {
                5 => M5_SCREEN_WIDTH,
                _ => SCREEN_WIDTH,
            };
    ...
    pub fn rect(&self, p: Vec2<u32>, width: u32, height: u32, color: Color) {
        for i in 0..=width {
            for j in 0..=height {
                self.point(p + Vec2::new(i, j), color);
            }
        }
    }

    /// Draws the frame of a rect with specified width + height with top-left at point p and specified color.
    pub fn frame(&self, p: Vec2<u32>, width: u32, height: u32, color: Color) {
        let top_left = p;
        ...
        self.line(bottom_left, bottom_right, color);
        self.line(top_left, bottom_left, color);
        self.line(top_right, bottom_right, color);
    }

    /// Draws a line from point p1 to point p2 with specified color.
    pub fn line(&self, p1: Vec2<u32>, p2: Vec2<u32>, color: Color) {
        ...
        if dy == 0 {
            for i in 0..=dx as u32 {
                self.point(Vec2::new(x1 + i, y1), color);
            }
        // Caso 2: linha vertical
        } else if dx == 0 {
            for i in 0..=dy as u32 {
                self.point(Vec2::new(x1, y1 + i), color);
            }
        }
        // Caso 3: linha diagonal
            ...
            loop {
                self.point(Vec2::new(x as u32, y as u32), color);
                if x == x1 && y == y1 {
                    break;
                }
